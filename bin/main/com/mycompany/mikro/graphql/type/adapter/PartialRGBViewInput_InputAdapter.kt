//
// AUTO-GENERATED FILE. DO NOT MODIFY.
//
// This class was automatically generated by Apollo GraphQL version '4.0.0'.
//
package com.mycompany.mikro.graphql.type.adapter

import com.apollographql.apollo.api.Adapter
import com.apollographql.apollo.api.CustomScalarAdapters
import com.apollographql.apollo.api.DoubleAdapter
import com.apollographql.apollo.api.NullableBooleanAdapter
import com.apollographql.apollo.api.NullableDoubleAdapter
import com.apollographql.apollo.api.NullableIntAdapter
import com.apollographql.apollo.api.NullableStringAdapter
import com.apollographql.apollo.api.Optional
import com.apollographql.apollo.api.json.JsonReader
import com.apollographql.apollo.api.json.JsonWriter
import com.apollographql.apollo.api.list
import com.apollographql.apollo.api.nullable
import com.apollographql.apollo.api.present
import com.mycompany.mikro.graphql.type.PartialRGBViewInput
import kotlin.IllegalStateException

public object PartialRGBViewInput_InputAdapter : Adapter<PartialRGBViewInput> {
  override fun fromJson(reader: JsonReader, customScalarAdapters: CustomScalarAdapters):
      PartialRGBViewInput = throw IllegalStateException("Input type used in output position")

  override fun toJson(
    writer: JsonWriter,
    customScalarAdapters: CustomScalarAdapters,
    `value`: PartialRGBViewInput,
  ) {
    if (value.collection is Optional.Present) {
      writer.name("collection")
      NullableStringAdapter.present().toJson(writer, customScalarAdapters, value.collection)
    }
    if (value.zMin is Optional.Present) {
      writer.name("zMin")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.zMin)
    }
    if (value.zMax is Optional.Present) {
      writer.name("zMax")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.zMax)
    }
    if (value.xMin is Optional.Present) {
      writer.name("xMin")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.xMin)
    }
    if (value.xMax is Optional.Present) {
      writer.name("xMax")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.xMax)
    }
    if (value.yMin is Optional.Present) {
      writer.name("yMin")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.yMin)
    }
    if (value.yMax is Optional.Present) {
      writer.name("yMax")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.yMax)
    }
    if (value.tMin is Optional.Present) {
      writer.name("tMin")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.tMin)
    }
    if (value.tMax is Optional.Present) {
      writer.name("tMax")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.tMax)
    }
    if (value.cMin is Optional.Present) {
      writer.name("cMin")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.cMin)
    }
    if (value.cMax is Optional.Present) {
      writer.name("cMax")
      NullableIntAdapter.present().toJson(writer, customScalarAdapters, value.cMax)
    }
    if (value.context is Optional.Present) {
      writer.name("context")
      NullableStringAdapter.present().toJson(writer, customScalarAdapters, value.context)
    }
    if (value.gamma is Optional.Present) {
      writer.name("gamma")
      NullableDoubleAdapter.present().toJson(writer, customScalarAdapters, value.gamma)
    }
    if (value.contrastLimitMin is Optional.Present) {
      writer.name("contrastLimitMin")
      NullableDoubleAdapter.present().toJson(writer, customScalarAdapters, value.contrastLimitMin)
    }
    if (value.contrastLimitMax is Optional.Present) {
      writer.name("contrastLimitMax")
      NullableDoubleAdapter.present().toJson(writer, customScalarAdapters, value.contrastLimitMax)
    }
    if (value.rescale is Optional.Present) {
      writer.name("rescale")
      NullableBooleanAdapter.present().toJson(writer, customScalarAdapters, value.rescale)
    }
    if (value.scale is Optional.Present) {
      writer.name("scale")
      NullableDoubleAdapter.present().toJson(writer, customScalarAdapters, value.scale)
    }
    if (value.active is Optional.Present) {
      writer.name("active")
      NullableBooleanAdapter.present().toJson(writer, customScalarAdapters, value.active)
    }
    if (value.colorMap is Optional.Present) {
      writer.name("colorMap")
      ColorMap_ResponseAdapter.nullable().present().toJson(writer, customScalarAdapters,
          value.colorMap)
    }
    if (value.baseColor is Optional.Present) {
      writer.name("baseColor")
      DoubleAdapter.list().nullable().present().toJson(writer, customScalarAdapters,
          value.baseColor)
    }
  }
}
